...
WITH effective_pairs AS (
    ...
    FROM exon_set AES
    JOIN exon_set TES ON TES.tx_ac=AES.tx_ac AND TES.alt_ac=AES.tx_ac AND TES.alt_aln_method='transcript'
...
SELECT T.hgnc,TES.exon_SET_id AS tes_exon_SET_id,AES.exon_SET_id AS aes_exon_SET_id,
       TES.tx_ac AS tx_ac,AES.alt_ac AS alt_ac,AES.alt_strand,EP.effective_alt_aln_method AS alt_aln_method,
...
SELECT T.hgnc,T.ac as tx_ac,AES.alt_ac,EP.effective_alt_aln_method AS alt_aln_method,AES.alt_strand,
